	      ...
	      )
{
  ...
  double t5 = t4 * t4; // cancels exactly against term in I-op.
  double t6 = CA * t5;
  double t10 = CA * Pi2;
			...
			double const& AlphaS)
{
  ...
  t9 = log(t8);
  t13 = log(t8 / x);
  return(0.4e1 * B_1 * CA * (-t13 * t3 + t3 * t6 - t3 * t9) * AlphaS / TwoPi);
		      ...
		      double const& AlphaS)
{
  ...
  t12 = P_gg_reg(x);
  t15 = -0.1e1 / t6;
  return(B_x * ((-0.2e1 * t5 + 0.2e1 * t7 + 0.2e1 * t9) * t12 + 0.4e1 * CA * (t15 * t5 - t15 * t7 - t15 * t9)) * AlphaS / TwoPi);
