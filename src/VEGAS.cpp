...
namespace MYVEGAS
{
  ...
  static int change_box_coord_after_pos (int k, gsl_monte_vegas_state * s, coord box[]);
...
#pragma omp parallel for num_threads(state->num_threads), shared(state,xl,jacbin)
	for (size_t k=0; k<state->boxes; ++k)
	  {
	      
	    // this thread iterates over all boxes with coordinates {k, ...}
	    int this_thread = omp_get_thread_num();
	    // init thread-local box coordinate
	    init_box_coord (state, box[this_thread]);
...
#pragma omp atomic
	    ...
	    while (change_box_coord_after_pos (1,state, box[this_thread]));
	  } //-- END OF PARALLEL FOR --//
  ...
  */
  static int
  change_box_coord_after_pos (int k, gsl_monte_vegas_state * s, coord box[])
  {
    int j = s->dim - 1;
