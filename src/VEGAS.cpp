  ...
  for (size_t it = 0; it < state->iterations; it++)
    {
      ...
      do
	    ...
	    for (size_t k = 0; k < calls_per_box; k++)
	      {
		// sets x, bin, state->bin_vol to current values
		random_point (x, bin, state->box, xl, state, r);
		double fval = jacbin * state->bin_vol * GSL_MONTE_FN_EVAL (f, x);
  ...
  for (size_t j = 0; j < s->dim; ++j)
    {
      ...
      z = ((box[j] + gsl_rng_uniform_pos (r)) / s->boxes) * s->bins;
      int k = z;
