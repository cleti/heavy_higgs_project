  ...
  for (int i=0;i<num_vals;++i)
    {
      ...
      double s_lo_B = vals_NLO_B[2*(num_vals+i)];//vals_LO[num_vals+i]+vals_NLO_gg[num_vals+i];
      double s_hi_B = vals_NLO_B[2*(2*num_vals+i)];//vals_NLO_gg[2*num_vals+i]+vals_LO[2*num_vals+i];


      cout << setw(10) <<  nlo_B    << setw(10) <<  s_lo_B     << setw(10) <<  s_hi_B  << endl;
      
      double nlo_lo_B = s_lo_B;
      double nlo_hi_B = s_hi_B;
      if (s_lo_B<nlo_B && s_hi_B>nlo_B)
      	{
      	  nlo_lo_B = nlo_B-s_lo_B;
      	  nlo_hi_B = s_hi_B-nlo_B;
      	}
      else (s_lo_B>nlo_B && s_hi_B<nlo_B)
      	{
      	  nlo_lo_B = nlo_B-s_hi_B;
      	  nlo_hi_B = s_lo_B-nlo_B;
      	}
      // else
      // 	{
      // 	  cout << endl << " invalid scale dependence! " << endl;
