...
/*   __BEGIN_DECLS */
  ...
  typedef struct {
    ...
    size_t bins_max; // maximal size of grid in each dimension
    double vol;      // total integation volume = P(i=1..dim) delx[i] 
    ...
    //std::atomic<double*> d;      // distribution of cumulated integrand values ( size = bins*dim )
    double* d;
    ...
    int    ** bin;   // which lies in these bin ( size = dim*num_threads )
    int    ** box;   // and in this box         ( size = dim*num_threads )
    double* bin_vol;  // relative volume of this bin (remember that VEGAS transforms to [0,1] integration intervals) ( size = num_threads )
  ...
  gsl_monte_vegas_state* gsl_monte_vegas_alloc(size_t num_threads, size_t dim);
